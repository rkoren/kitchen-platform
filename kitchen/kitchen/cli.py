...
def _menu_json_schema() -> dict:
    """The Menu manifest as a standalone JSON Schema document (draft 2020-12)."""
...
@menu_app.command("schema")
def menu_schema(
    out: Annotated[
        str | None,
        typer.Option("--out", "-o", metavar="PATH", help="Write schema to a file instead of stdout"),
    ] = None,
) -> None:
    """Export the menu.yaml JSON Schema (draft 2020-12).

    With no --out, prints the schema to stdout so it can be redirected or piped
    (e.g. `kitchen menu schema > menu.schema.json`). Use the emitted file as a `$schema`
    reference for editor validation and autocompletion of menu YAML. The committed
    `docs/kitchen/menu.schema.json` is generated by this command (a test keeps it in sync).
    """
    import json

    text = json.dumps(_menu_json_schema(), indent=2) + "\n"
    if out:

...
class RDSSpec(BaseModel):
    ...
    db_subnet_group_name: str | None = None  # use an EXISTING subnet group by name
    subnet_ids: list[str] = []  # subnets (>=2 AZs) → recipes generates an aws_db_subnet_group
    ...
    @model_validator(mode="after")
    def _validate_subnets(self) -> "RDSSpec":
        if self.subnet_ids and self.db_subnet_group_name:
            raise ValueError(
                f"RDS '{self.name}': set either subnet_ids (recipes makes the subnet group) or "
                "db_subnet_group_name (an existing one), not both."
            )
        if self.subnet_ids and len(self.subnet_ids) < 2:
            raise ValueError(
                f"RDS '{self.name}': subnet_ids needs at least two subnets (in different AZs) — "
                "RDS requires a DB subnet group spanning >=2 availability zones."
            )
        return self
